...
function AdminConsole(_ref24) {
  ...
  const _ref25 = window.firebaseServices || {},
    ...
    runTransaction = _ref25.runTransaction,
    where = _ref25.where;
...
function UserSettingsPage(_ref34) {
  ...
  return React.createElement("div", {
    ...
    className: "su-account-hero relative overflow-hidden rounded-[2rem] p-5 sm:p-7 bg-gradient-to-br from-slate-950 via-indigo-950 to-sky-800 text-white shadow-[0_30px_90px_-35px_rgba(15,23,42,.8)]"
  }, React.createElement("div", {
    ...
    className: "space-y-6"
  }, React.createElement(window.StudyUI.StylePicker, null), React.createElement("div", null, React.createElement("div", {
...
function App() {
  ...
  useEffect(() => {
    ...
    const previousOverflow = document.body.style.overflow;
    const closeOnEscape = event => {
      if (event.key === 'Escape' && !document.querySelector('.su-overlay')) setIsComposerOpen(false);
    };
    document.body.style.overflow = 'hidden';
    window.addEventListener('keydown', closeOnEscape);
    return () => {
  ...
  const displayedItems = items.filter(item => {
    ...
    if (viewMode === 'current' && homeScope === 'tomorrow' && (item.type !== 'homework' || item.forceExpired || toLocalDateKey(item.dueDate) !== window.StudyCore.tomorrow(globalNow))) return false;
  ...
  const isVisible = componentName => {
    ...
    if (layoutMode === 'normal') return componentName === 'cards' || componentName === 'list';
  ...
  }, React.createElement(window.StudyUI.ProfileMenu, {name:displayName,isAdmin:isAdmin,onSettings:()=>{setIsComposerOpen(false);setAppMode('personal');setViewMode('settings');},onInbox:()=>{setAppMode('personal');setViewMode('collaboration');setCollabTab('inbox');},onAdmin:()=>{setIsComposerOpen(false);setViewMode('admin');}}), React.createElement("div", {
  ...
  }, ['timetable','calendar','timetable_edit'].includes(viewMode) && React.createElement(window.StudyUI.ScheduleTabs,{value:viewMode,onChange:v=>setViewMode(v)}), viewMode === 'collaboration' ? React.createElement(CollaborationWorkspace, {
    collab: collab,
    user: user,
    items: items,
    subjects: subjects,
  ...
  }, React.createElement("span", null, counts.weekProgressPercent, "%")))), React.createElement("div",{className:"su-quick-filters col-span-2 sm:col-span-3"},
React.createElement("button",{type:"button",onClick:()=>openQuickView('tomorrow')},"明天待交 ",counts.tomorrow),
React.createElement("button",{type:"button",onClick:()=>openQuickView('today')},"今日待交 ",counts.today),
React.createElement("button",{type:"button",onClick:()=>openQuickView('test')},"測驗及其他 ",counts.test))), viewMode === 'current' && appMode === 'personal' && isVisible('timetable') && React.createElement(CurrentLessonWidget, {
    config: timetableConfig,
    t: t,
  ...
  }, viewMode === 'history' ? '沒有已完成的紀錄' : searchQuery ? '搵唔到相關功課' : homeScope === 'tomorrow' ? '明天暫時沒有未完成的功課；其他日期可查看「全部待辦」。' : '太好了！任務清單空空如也')) : displayedItems.map((item, index) => {
    var _subjects$find6;
    ...
    name: "plus",
    className: "w-7 h-7"
  })), React.createElement("nav", {className: "bottom-nav su-nav", "aria-label": "主要導覽"},
React.createElement(NavItem, {icon:"book-open",label:"主頁",active:['current','history'].includes(viewMode)&&appMode==='personal',onClick:()=>{setIsComposerOpen(false);setViewMode('current');setAppMode('personal');setIsListVisible(true);},t:t}),
React.createElement(NavItem, {icon:"calendar-clock",label:"時間表",active:['timetable','timetable_edit','calendar'].includes(viewMode),onClick:()=>{setIsComposerOpen(false);setViewMode('timetable');setAppMode('personal');setIsListVisible(true);},t:t}),
React.createElement(NavItem, {icon:"users",label:"群組",active:viewMode==='collaboration'||appMode==='class',onClick:()=>{setIsComposerOpen(false);setViewMode('collaboration');setCollabTab('groups');setAppMode('personal');},t:t})), editingItem !== null && editingItem !== void 0 && editingItem.collabRefId ? React.createElement(CollaborationPersonalNote, {
    item: editingItem,
...
try {
  ...
  const loadingScreen = document.getElementById('loading-screen');
  if (loadingScreen) {
    loadingScreen.innerHTML = "<div style='max-width:420px;padding:22px;text-align:center'><div style='font-size:42px;margin-bottom:10px'>⚠️</div><h2 style='font-size:1.2rem;font-weight:800;margin-bottom:8px;color:#ef4444'>主畫面啟動失敗</h2><p style='font-size:13px;color:#64748b;line-height:1.6'>請重新整理；如仍未能載入，請確認同一層的所有新版檔案已上傳。</p></div>";
  }
}
})();