...
(function () {
  ...
  const errorText = e => ({
    'spark/not-found': '免費協作資料尚未準備好，請先開啟「免費版設定指南」。',
    'spark/unavailable': '目前未能連接免費協作資料，請檢查網絡後重試。',
    'functions/not-found': '免費版不需要 Functions；請開啟「免費版設定指南」。',
    'functions/unavailable': '目前未能連接免費協作資料，請檢查網絡後重試。',
    'functions/unauthenticated': '登入已過期，請重新登入。',
    'permission-denied': 'Firebase 規則尚未更新或你沒有此操作權限。首次使用請開啟「免費版設定指南」完成規則發布。',
    'resource-exhausted': '今日 Firebase 免費額度已用盡，請等額度重設後再試；不需要升級付款。',
    'unavailable': '目前未能連接 Firebase，請檢查網絡後重試。',
    'failed-precondition': 'Firestore 尚未準備好，可能需要建立索引或完成資料庫設定。' + (e && e.message ? ' 詳細資料：' + e.message : ''),
  ...
  function Dialog(_ref7) {
    ...
    useEffect(() => {
      ...
      const key = e => {
        if (document.querySelector('.su-overlay')) return;
        if (e.key === 'Escape') {
          var _close$current;
          e.stopPropagation();
  ...
  function useCollaboration(user) {
    ...
    const api = async function (action) {
      ...
      if (!(svc !== null && svc !== void 0 && svc.callCollab)) throw Object.assign(new Error('免費協作資料尚未就緒'), {
        code: 'spark/unavailable'
      });
    ...
    useEffect(() => {
      ...
      api('bootstrap').then(() => {
        ...
        listen(f.doc(f.db, 'spark_accounts', user.uid), snap => {
          setAccount(snap.data());
          setLoading(false);
        });
        listen(f.query(f.collection(f.db, 'spark_groups'), f.where('members', 'array-contains', user.uid)), snap => setGroups(snap.docs.map(x => ({
          ...x.data(),
          id: x.id
        }))));
        listen(f.collection(f.db, 'spark_preferences', user.uid, 'groups'), snap => setPreferences(Object.fromEntries(snap.docs.map(x => [x.id, x.data()]))));
        let requests = [],
          infos = [];
        const refreshInbox = () => setInbox([...requests, ...infos].sort((a, b) => b.createdAt - a.createdAt).slice(0, 100));
        listen(f.query(f.collection(f.db, 'spark_requests'), f.where('recipientUid', '==', user.uid)), snap => {
          requests = snap.docs.map(x => ({
            ...x.data(),
            id: x.id
          }));
          refreshInbox();
        });
        listen(f.query(f.collection(f.db, 'spark_inbox', user.uid, 'entries'), f.orderBy('createdAt', 'desc'), f.limit(100)), snap => {
          infos = snap.docs.map(x => ({
            ...x.data(),
            id: x.id
          }));
          refreshInbox();
        });
        unsubs.push(window.studySpark.startSync(e => {
          if (active) setError(errorText(e));
        }));
        listen(f.query(f.collection(f.db, 'spark_assignments'), f.where('ownerUid', '==', user.uid)), snap => setIssued(snap.docs.map(x => ({
          ...x.data(),
          id: x.id
  ...
  function AccountCard(_ref8) {
    ...
    return React.createElement("div", {
      className: "co-card co-workspace su-account-code"
    }, React.createElement("div", {
      ...
      name: "copy"
    }), "\u8907\u88FD\u7DE8\u865F")))), (a === null || a === void 0 ? void 0 : a.disabled) && React.createElement("p", {
      className: "co-notice error"
    }, "\u5354\u4F5C\u529F\u80FD\u5DF2\u88AB\u7E3D\u7BA1\u7406\u54E1\u66AB\u505C\u3002"), collab.error && React.createElement("div", {
      className: "co-notice error"
    }, React.createElement("p", null, collab.error), React.createElement("div", {
      className: "co-actions"
    }, React.createElement(Button, {
      onClick: collab.retry
    }, "\u91CD\u65B0\u9023\u63A5"), React.createElement("a", {
      className: "co-btn",
      href: "./setup.html",
      target: "_blank",
      rel: "noopener"
  ...
  function Composer(_ref9) {
    ...
    return React.createElement(Dialog, {
      ...
      className: "co-field"
    }, "\u985E\u578B", React.createElement(window.StudyUI.Select, {
      className: "co-input",
      value: form.kind,
      ...
      className: "co-field"
    }, "\u79D1\u76EE", React.createElement(window.StudyUI.SubjectInput, {
      className: "co-input",
      ...
      className: "co-field"
    }, form.kind === 'notice' ? '公告日期' : form.kind === 'activity' ? '活動日期' : '交期／日期', React.createElement(window.StudyUI.DateInput, {
      type: "date",
      className: "co-input",
      value: form.dueDate,
      ...
      className: "co-field"
    }, "\u512A\u5148\u6B21\u5E8F", React.createElement(window.StudyUI.Select, {
      className: "co-input",
      value: form.priority,
  ...
  function GroupForm(_ref0) {
    ...
    return React.createElement(Dialog, {
      ...
      className: "co-muted"
    }, mode === 'invite' ? '五個英文字母；先核對姓名再邀請。' : '加入需經群主確認；接受後預設同步未到期功課。')), found && React.createElement("div", {
      className: "co-notice"
  ...
  function GroupDetail(_ref10) {
    ...
    useEffect(() => {
      ...
      setError('');
      const f = window.firebaseServices;
      return f.onSnapshot(f.query(f.collection(f.db, 'spark_assignments'), f.where('groupId', '==', group.id), f.where('audience', 'array-contains', uid)), snap => setTasks(snap.docs.map(x => ({
        ...x.data(),
        id: x.id
      })).sort((a, b) => b.createdAt - a.createdAt).slice(0, more)), e => setError(errorText(e)));
    }, [group.id, more, uid]);
    ...
    return React.createElement("div", {
      ...
      className: "co-notice"
    }, pref.sync ? '同步已開啟：開啟網站時，新功課會加入個人手冊。' : '同步已關閉：可逐份加入個人手冊。', " \u5B8C\u6210\u72C0\u614B\u53EA\u6703\u66F4\u6539\u81EA\u5DF1\u3002"), error && React.createElement("div", {
      className: "co-notice error"
  ...
  function Admin(_ref16) {
    ...
    return React.createElement("div", {
      ...
      className: "co-muted"
    }, "\u514D\u8CBB\u7248\u5354\u4F5C\u6B0A\u9650\u7BA1\u7406\u3002\u522A\u9664\u767B\u5165\u5E33\u6236\u53CA\u4FEE\u6539\u96FB\u90F5\u8ACB\u5230 Firebase \u63A7\u5236\u53F0\u3002")), React.createElement("div", {
      className: "co-tabs",
      role: "tablist",
      "aria-label": "\u7BA1\u7406\u5206\u985E"
  ...
  function Workspace(_ref21) {
    ...
    if (!collab.account) return React.createElement("div", {
      ...
      target: "_blank",
      rel: "noopener"
    }, "\u514D\u8CBB\u7248\u8A2D\u5B9A\u6307\u5357"), React.createElement(Button, {
      onClick: onLegacy
    }, "\u539F\u6709\u73ED\u7D1A")));
    ...
    return React.createElement("div", {
      ...
      role: "tablist",
      "aria-label": "\u5354\u4F5C\u5DE5\u4F5C\u5340"
    }, [['groups', '我的群組'], ['issued', '我發放的'], ...(!['groups','issued'].includes(tab) ? [[tab, {inbox:'收件匣',account:'帳戶編號',admin:'總管理'}[tab] || '詳情']] : [])].map(_ref22 => {
      ...
      className: "co-kicker"
    }, "StudyOS 3.6.0 Spark"), React.createElement("h2", null, "\u4E00\u8D77\u6574\u7406\uFF0C\u5404\u81EA\u5B8C\u6210\u3002"), React.createElement("p", {
      className: "co-muted"
